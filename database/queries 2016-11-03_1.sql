/* Modificando la funcion para regresar stock considerando sucursal */
CREATE OR REPLACE FUNCTION invalidate(d_sale_serie INTEGER,d_sale_serial INTEGER) returns void as $$
	...
	SELECT sales.id, sales.branch_id INTO v_sale_id, v_branch_id FROM sales WHERE sales.serie = d_sale_serie AND sales.serial_number = d_sale_serial;
