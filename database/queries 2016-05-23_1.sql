/* Crear la función para guardar un pedido con sus respectivos detalles */
CREATE OR REPLACE FUNCTION save_purchase_order(JSON, JSON[])
  RETURNS INTEGER AS
$BODY$
DECLARE
...
BEGIN
	INSERT INTO purchase_orders (code, description, start_date, finish_date, paid_date, active, supplier_id, branch_detail_id)
	VALUES (
		($1->>'code'),
		($1->>'description'),
	...
	LOOP
		...
		ELSEIF ($1->>'product_id') IS NOT NULL THEN
