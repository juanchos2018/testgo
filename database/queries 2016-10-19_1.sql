...
/* Restructurando Función save_transfers para guardar traslados, guias */
CREATE OR REPLACE FUNCTION save_transfer(json, json[], json[])
  RETURNS integer AS
$BODY$
DECLARE
    v_json JSON;
...
BEGIN
    ...
    INSERT INTO transfers (transfer_date, total_qty, branch_detail_origin_id, branch_detail_target_id, shuttle_reason_id, approved_state, approved_by, approved_at, registered_by)
    VALUES (
        ($1->>'transfer_date')::DATE,
        ($1->>'total_qty')::INTEGER,
        (SELECT bd.id FROM branch_details bd WHERE bd.branch_id = ($1->>'branch_origin_id')::INTEGER AND bd.company_id = ($1->>'company_origin_id')::INTEGER),
        (SELECT bd.id FROM branch_details bd WHERE bd.branch_id = ($1->>'branch_target_id')::INTEGER AND bd.company_id = ($1->>'company_target_id')::INTEGER),
        ($1->>'shuttle_reason_id')::INTEGER,
    ...
    LOOP
        ...
        IF v_stock_id IS NULL THEN
            RAISE EXCEPTION 'No se pudo actualizar el stock para código de barras (%) y sucursal (%)', v_product_barcode_id, $1->>'branch_origin_id';
        END IF;
