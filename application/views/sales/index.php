                   ...
                   <!--  <div class="col-sm-12">
                    ...
                    <div class="col-sm-12">
						<?php echo erp_datatable(
							'sale in sales',
							array(
                                'labels' => array('Item', 'Cod', 'Descripcion','Stock Tienda.','Stock Almacen.','Cant. Vend','Venta Neta'),
                                'fields' => array('{{sale.id}}', '{{sale.code}}', '{{sale.description}}', '{{sale.store_stock}}','{{sale.depot_stock}}','{{sale.qty_sold}}','{{sale.total | currency:"S/"}}')
							),
                            'multiple',
							'selected'
						); ?>
...
<script>
    angularScope(function ($scope) {
        $scope.sales = <?php echo json_encode($records); ?>.extendEach({selected: false});
    });
</script>