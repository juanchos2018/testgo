                ...
                <div class="row">
                    ...
                    <div class="col-sm-12">
						<?php echo erp_datatable(
							'user in users',
							array(
                                'labels' => array('Nombres y Apellidos', 'Usuario', 'E-mail'),
                                'fields' => array('{{user.full_name}}', '{{user.username}}', '<a ng-href="mailto:{{user.email}}">{{user.email}}</a>')
							),
                            'multiple',
							'selected'
						); ?>
...
<script>
    angularScope(function ($scope) {
        $scope.users = <?php echo json_encode($records); ?>.extendEach({selected: false});
    });
</script>
