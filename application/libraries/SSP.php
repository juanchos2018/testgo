...
class SSP {
	...
	public function data_output ( $columns, $data )
	{
		...
		for ( $i=0, $ien=count($data) ; $i<$ien ; $i++ ) {
			...
			for ( $j=0, $jen=count($columns) ; $j<$jen ; $j++ ) {
				...
				if (strpos($column['db'], ' AS ') !== FALSE) { // AS siempre debe ser mayúscula
					$column['db'] = end(explode(' AS ', $column['db']));
				} elseif (strpos($column['db'], '.') !== FALSE) {
	 ...
	 */
	public function filter ( $request, $columns, &$bindings, $_where = '' )
	{
		$globalSearch = array();
		$columnSearch = array();
		$dtColumns = $this->pluck( $columns, 'dt' );
		...
		if ( isset($request['search']) && $request['search']['value'] != '' ) {
			...
			for ( $i=0, $ien=count($request['columns']) ; $i<$ien ; $i++ ) {
				...
				if ( $requestColumn['searchable'] == 'true' ) {
					if (strpos($column['db'], ' AS ') !== FALSE) { // AS siempre debe ser mayúscula
						$column['db'] = current(explode(' AS ', $column['db']));
					}
	 ...
	 */
	public function simple ( $request, $sql_details, $table, $primaryKey, $columns, $where = '' )
	{
		$bindings = array();
		...
		$where = $this->filter( $request, $columns, $bindings, $where );
