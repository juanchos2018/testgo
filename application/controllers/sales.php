			...
			foreach ($this->input->post('invoices') as $invoice) {
				...
				if ($invoice['voucher'] == '07') {
					$cabecera = array(
						...
						'tipMoneda' => 'PEN',
						'codMotivo' => '01',
						'desMotivo' => 'DEVOLUCION DE ITEM',
						'tipDocAfectado' => '03',
						'numDocAfectado' => "{$invoice['reference2']}",
				...
				if($invoice['company_id'] == 1) {
				    // LFA
					$ruc = '20532437548';
					$file = __DIR__ .'/opt/SFS_LFA/'.$ruc.'-'.$vouchertype.'-'.$serie.'-'.$serial.'.JSON';
				} else {
					// GAFCO
					$ruc = '20302573086';
					$file = __DIR__ .'/opt/SFS_GAFCO/'.$ruc.'-'.$vouchertype.'-'.$serie.'-'.$serial.'.JSON';
				}
