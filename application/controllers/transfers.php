...
class Transfers extends MY_Controller {
	...
	public function index()
	{
		...
		$data['records'] = $this->transfer_model->get_all(array(
			'branch_origin_id' => $this->session->userdata('user_branch')
		));
	...
	public function detail($id)
	{
		...
		$detail = $this->transfer_model->get($id);

		if ($detail !== FALSE) {
			$detail['details'] = $this->transfer_model->get_details($id);

			$this->load->view('transfers/detail', array('detail' => $detail));
		} else {
	...
	public function save()
	{
		...
		if ($this->form_validation->run() !== FALSE) {
			...
			$inserted = $this->transfer_model->save();
