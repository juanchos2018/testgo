window.angular.module('ERP').directive('erpShuttleReasonsChooser', [
	...
	function ($timeout, $filter, Ajax, siteUrl, bootbox) {
		return {
			...
			controller: ['$scope', '$element', function($scope, $element) {
				...
				Ajax.get(siteUrl('shuttle_reasons/simple_list')).then(function (res) {
				...
				$scope.showForm = function () {
						...
						<div class="alert alert-danger alert-dismissible" style="display:none" role="alert">
							...
							<form class="form-horizontal" autocomplete="off">
								...
								<div class="form-group required">
									<label class="col-sm-3 control-label">Motivos de traslado</label>
									<div class="col-sm-9">
					...
					modalForm.one('show.bs.modal', function () {
						modalForm.find('form').submit(function (e) {
							...
							Ajax.post(siteUrl('shuttle_reasons/save'), new FormData(this)).then(function (res) {
								if (is.json(res.data)) {
									if ('ok' in res.data) {
										...
										$scope.options.push({
											id: insertId,
											text: modalForm.find('input[name="description"]').val()
										});
						...
						modalForm.find('input[name="description"]').focus();
					}).modal('show');
				};
			}],
            template: `
				<div ng-class="{'input-group': addButton}">
					...
					<span class="input-group-btn" ng-if="addButton">
						<button class="btn btn-success" type="button" title="Nuevo motivo" data-tooltip ng-click="showForm()">
