...
window.angular.module('ERP').controller('TransfersAddCtrl', [
	...
	function ($scope, $rootScope, $window, $filter, $location, $timeout, Page, Settings, FileHandler, Auth, Ajax, Session, baseUrl, siteUrl, bootbox, riot, $, moment) {
		...
		$scope.finish = function () {
			if ($scope.data.filter(function (value) {
				...
				return (parseInt(value['CANT.'],10 || 0) > parseInt(value.STOCK_O,10));
			}).length) {
				bootbox.dialog({
					title: 'Se encontraron datos no válidos',
					message: 'Algunos productos no tienen stock suficiente para el traslado.',
					buttons: {
						ok: {
							label: 'Aceptar',
							className: 'btn-danger'
		...
		$scope.save = function (newInvoices) {
			...
			Ajax.post(siteUrl('transfers/save'), {
